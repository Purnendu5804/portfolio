...
- **Section order on home page** — Hero → Skills → Experience → Projects → Uses → Contact → Footer
...
### 7. Visitor Counter (Firebase)

Edit `src/firebase/config.ts` — replace env var values in `.env`:
...
| **Never add new npm packages** | The dependency list is minimal and curated |
